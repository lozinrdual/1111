...
function AnimatedLink({ text, href }: { text: string; href: string }) {
  ...
  return (
    <Link
      href={href}
      className="flex items-center justify-between w-full max-w-lg bg-cta px-0 py-3 md:py-4 border-t border-border text-sidebar-foreground font-medium text-lg md:text-xl hover:bg-secondary-cta-hover transition-all duration-300 ease-out group cursor-pointer"
    >
      ...
      <svg
        className="w-5 h-5 md:w-6 md:h-6 text-sidebar-foreground transition-transform duration-300 group-hover:rotate-[-30deg]"
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
      >
...
function AnimatedButton({ text, href }: { text: string; href: string }) {
  ...
  return (
    <Link
      href={href}
      className="w-full md:max-w-2xl px-6 md:px-8 py-4 md:py-5 bg-secondary-cta text-chart-5 tracking-tight font-medium text-[18px] md:text-xl rounded-lg transition-all duration-300 ease-out flex items-center justify-center gap-3 group"
    >
      <span className="relative overflow-hidden inline-flex">
        {/* Primary text - slides down on hover */}
        <span className="flex">
      ...
      <svg
        className="w-6 h-6 text-chart-5 group-hover:translate-x-1 transition-transform duration-300 ease-out"
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
      >
...
const services = [
  {
    ...
    icon: (
      <svg
        className="w-30 h-30 md:w-30 md:h-30 text-cta"
...
const PatternCircles = () => (
  <svg
    className="absolute inset-0 w-full h-full opacity-20 text-cta"
    viewBox="0 0 400 300"
    xmlns="http://www.w3.org/2000/svg"
  >
    <style>{`
    ...
    `}</style>
    <circle cx="80" cy="60" r="30" fill="currentColor" className="circle-animate" />
    <circle cx="280" cy="100" r="25" fill="currentColor" className="circle-animate" style={{ animationDelay: "1s" }} />
    <circle cx="200" cy="200" r="35" fill="currentColor" className="circle-animate" style={{ animationDelay: "2s" }} />
  </svg>
)
...
const PatternStripes = () => (
  ...
  >
    ...
    `}</style>
    <g className="stripe">
      <rect x="0" y="0" width="20" height="300" fill="currentColor" />
      <rect x="40" y="0" width="20" height="300" fill="currentColor" />
      <rect x="80" y="0" width="20" height="300" fill="currentColor" />
      <rect x="120" y="0" width="20" height="300" fill="currentColor" />
      <rect x="160" y="0" width="20" height="300" fill="currentColor" />
      <rect x="200" y="0" width="20" height="300" fill="currentColor" />
      <rect x="240" y="0" width="20" height="300" fill="currentColor" />
      <rect x="280" y="0" width="20" height="300" fill="currentColor" />
      <rect x="320" y="0" width="20" height="300" fill="currentColor" />
      <rect x="360" y="0" width="20" height="300" fill="currentColor" />
    </g>
  </svg>
)
...
export function ServiceCards() {
  ...
  return (
    <div>
      <div ref={containerRef} className="relative mt-8 md:mt-12 lg:mt-16 space-y-8 md:space-y-0">
        ...
        >
          <div className="p-8 md:p-8 bg-background backdrop-blur-sm border border-border rounded-2xl flex-1 min-h-80 md:min-h-96 flex flex-col justify-between md:-rotate-2 hover:rotate-0 transition-transform duration-500 ease-out">
            <div
              className={`${services[0].bgColor} rounded-xl p-6 mb-6 relative overflow-hidden min-h-48 flex items-center justify-center`}
            >
              <PatternCircles />
              <div className="relative z-10 text-cta text-3xl font-bold opacity-20">●●●</div>
            </div>
            <div>
        ...
        <div className="md:grid md:grid-cols-2 md:gap-8 md:relative md:ml-20 space-y-8 md:space-y-0">
          ...
          <div
            className="p-8 md:p-8 bg-background backdrop-blur-sm border border-chart-border rounded-2xl flex flex-col justify-between min-h-80 md:min-h-72 md:rotate-1 hover:rotate-0 transition-transform duration-500 ease-out"
            style={{
          ...
          >
            ...
            >
              <PatternWaves />
              <div className="relative z-10 text-cta text-2xl opacity-20">∿∿∿</div>
            </div>
            <div>
          ...
          <div
            className="p-8 md:p-8 bg-background backdrop-blur-sm border border-border rounded-2xl flex flex-col justify-between min-h-80 md:min-h-72 md:-rotate-1 hover:rotate-0 transition-transform duration-500 ease-out md:mt-12"
            style={{
        ...
        {/* Reduced bottom margin spacing */}
        <div className="mt-4 md:mt-8 lg:mt-12 flex justify-center md:hidden">
